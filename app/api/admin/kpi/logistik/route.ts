...
import {
  upsertKpiSalesRetailHariKerja,
  upsertKpiSalesRetailInputs,
  type KpiSalesRetailInputRow,
...
// KPI Logistik memakai tabel penyimpanan yang sama (kpi_sales_retail_input) dengan
// row_key khusus "log*", jadi cukup reuse fungsi upsert Sales & Retail.
export async function POST(request: Request) {
  ...
  try {
    ...
    const rows: KpiSalesRetailInputRow[] = rawRows
      .map((r) => {
        ...
        } satisfies KpiSalesRetailInputRow;
      })
      .filter((r): r is KpiSalesRetailInputRow => r !== null);

    const hariKerja = parsePositiveInt(body.hariKerja);
    if (hariKerja) await upsertKpiSalesRetailHariKerja(month, year, hariKerja);

    await upsertKpiSalesRetailInputs(employeeId, month, year, rows);

    return NextResponse.json({ message: "Penilaian KPI Logistik berhasil disimpan." });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Gagal menyimpan penilaian KPI.";
    console.error("Save KPI Logistik error", error);
