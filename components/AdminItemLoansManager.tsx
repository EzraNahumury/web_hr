...
export default function AdminItemLoansManager({ initialRows, employees }: Props) {
  ...
  const filteredEmployees = useMemo(() => {
    ...
    return employees.filter(
      (e) => e.name.toLowerCase().includes(q) || e.nip.toLowerCase().includes(q),
    );
  }, [empSearch, employees]);
  ...
  function handleSubmit() {
    ...
    if (!employeeId) {
      setToast({ type: "error", text: "Pilih karyawan terlebih dahulu." });
      return;
    }
    ...
    startTransition(async () => {
      try {
        const res = await fetch("/api/admin/item-loans", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ employeeId, items: cleanItems, loanDate: loanDate || null, note: note || null }),
        });
        const result = (await res.json()) as { message?: string };
        if (!res.ok) throw new Error(result.message || "Gagal menyimpan.");
        setToast({ type: "success", text: result.message || "Peminjaman tersimpan." });
        resetForm();
        router.refresh();
      } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-[28px] border border-[#ead7ce] bg-white p-6 shadow-[0_18px_50px_rgba(15,23,42,0.06)]">
        <h2 className="text-lg font-semibold text-[#241716]">Catat Peminjaman Barang</h2>
        <p className="mt-1 text-sm text-[#7a6059]">Pilih karyawan, isi barang yang dipinjam (bisa lebih dari 1), tanggal, dan keterangan.</p>

        <div className="mt-5 grid gap-4 lg:grid-cols-2">
          {/* Employee picker */}
          <div>
            <label className="block text-[13px] font-semibold text-[#6f5a54]">Nama Karyawan</label>
            <div ref={empRef} className="relative mt-2">
              <input
                type="text"
                value={empOpen ? empSearch : selectedEmployee ? `${selectedEmployee.name} (${selectedEmployee.nip})` : ""}
                onChange={(e) => { setEmpSearch(e.target.value); setEmpOpen(true); }}
                onFocus={() => { setEmpSearch(""); setEmpOpen(true); }}
                  ...
                  {filteredEmployees.length === 0 ? (
                    ...
                    filteredEmployees.slice(0, 50).map((e) => (
                      <button
                        key={e.id}
                        type="button"
                        onClick={() => { setEmployeeId(e.id); setEmpOpen(false); setEmpSearch(""); }}
                        className={`block w-full px-4 py-2.5 text-left text-sm hover:bg-[#fff0e8] ${e.id === employeeId ? "bg-[#fff0e8] font-semibold text-[#8f1d22]" : "text-[#241716]"}`}
                      >
                ...
                rows.map((r, i) => (
                  <tr key={r.id} className="border-b border-[#f1e5de] last:border-b-0 align-top">
                    ...
                    <td className="px-5 py-4">
                      <div className="font-semibold text-[#241716]">{r.employeeName}</div>
                      <div className="text-xs text-[#9e7a72]">{r.nip}</div>
                    </td>
                    <td className="px-5 py-4">
                    ...
                    <td className="px-5 py-4 tabular-nums text-[#241716]">{formatDate(r.loanDate)}</td>
